...
<?php endif; ?>
            ...
            <tr>
                ...
                <td>
                    <select name="published">
                        <option value="0" <?php if ($this->job_post->published == 0) {
                            echo 'selected="selected"';
                        } ?>><?php echo JText::_('COM_JOBBOARD_JNO'); ?></option>
                        <option value="1" <?php if ($this->job_post->published == 1) {
                            echo 'selected="selected"';
                        } ?>><?php echo JText::_('COM_JOBBOARD_JYES'); ?></option>
                    </select>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td>
                    <select name="questionnaire_id" id="questionnaire_id">
                        <option value="0" <?php if (!$newjob) {
                            if ($this->job_post->questionnaire_id == 0) echo 'selected="selected"';
                        } ?>><?php echo JText::_('COM_JOBBOARD_NOASSIGNEDQNAIRE'); ?> </option>
                        <?php if (count($this->questionnaires) > 0) : ?>
                            <?php foreach ($this->questionnaires as $jq) : ?>
                                <option value="<?php echo $jq['id'] ?>" <?php if (!$newjob) {
                                    if ($this->job_post->questionnaire_id == $jq['id']) echo 'selected="selected"';
                                } ?>> <?php echo $jq['title'] ?>&nbsp;(<?php echo $jq['name'] ?>)
                                </option>
                            <?php endforeach ?>
                        <?php endif ?>
                    </select> <br />
        ...
        <tr>
            ...
            <td>
                ...
                <?php switch ($this->config->long_date_format) {
                    case 0:
                        echo JHTML::_('date', $this->job_post->post_date, $this->day_format . ' ' . $this->month_short_format . ', ' . $this->year_format);
                        break;
                    case 1:
                        echo JHTML::_('date', $this->job_post->post_date, $this->month_short_format . ' ' . $this->day_format . ', ' . $this->year_format);
                        break;
                    case 2:
                        echo JHTML::_('date', $this->job_post->post_date, $this->year_format . ', ' . $this->day_format . ' ' . $this->month_short_format);
                        break;?>
                    <?php
                }; ?>
            ...
            <select name="category" id="category">
                <?php foreach ($this->categories as $category) : ?>
                    <?php if ($category->enabled = 1 && $category->id > 1) : ?>
                        <option value="<?php echo $category->id ?>" <?php if ($category->id == $this->job_post->category) {
                            echo 'selected="selected"';
        ...
        <td>
            <select name="department" id="department">
                <?php foreach ($this->departments as $department) : ?>
                    <option value="<?php echo $department->id ?>" <?php if ($department->id == $this->job_post->department) {
                        echo 'selected="selected"';
                ...
                <select name="country" id="country">
                    <?php foreach ($this->countries as $country) : ?>
                        <?php if ($country->country_id == 266) : ?>
                            <option value="<?php echo $country->country_id ?>" <?php if ($country->country_id == $this->job_post->country) {
                                echo 'selected="selected"';
                            } ?>><?php echo JText::_($country->country_name); ?></option>
                        <?php else: ?>
                            <option value="<?php echo $country->country_id ?>" <?php if ($country->country_id == $this->job_post->country) {
                                echo 'selected="selected"';
        ...
        <td>
            <select name="career_level" id="career_level">
                <?php foreach ($this->careers as $career) : ?>
                    <option value="<?php echo $career->id ?>" <?php if ($career->id == $this->job_post->career_level) {
                        echo 'selected="selected"';
                    } ?>><?php echo $career->description; ?></option>
                <?php endforeach; ?>
            </select>
        </td>
    </tr>
        ...
        <td>
            <select name="education_level" id="education_level">
                <?php foreach ($this->education as $ed) : ?>
                    <option value="<?php echo $ed->id ?>" <?php if ($ed->id == $this->job_post->education) {
                        echo 'selected="selected"';
                    } ?>><?php echo $ed->level; ?></option>
                <?php endforeach; ?>
            </select>
        </td>
    </tr>
        ...
        <td>
            <select name="job_type" id="job_type">
                ...
                <?php foreach ($job_types as $job_type) : ?>
                    <option value="<?php echo $job_type ?>" <?php if ($job_type == $this->job_post->job_type) {
                        echo 'selected="selected"';
                    } ?>><?php echo JText::_($job_type); ?></option>
                <?php endforeach; ?>
            </select>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <input name="salary" id="salary" type="text" value="<?php echo $this->job_post->salary; ?>" />
            <small><?php echo ' (' . JText::_('COM_JOBBOARD_EXAMPLE_ABBR') . ' ' . JText::_('COM_JOBBOARD_SAL_EG') . ') - ' . JText::_('COM_JOBBOARD_BLANK_IF_NEG'); ?></small>
        </td>
    </tr>
    </table>
    <br />
