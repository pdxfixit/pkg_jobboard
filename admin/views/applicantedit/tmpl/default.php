            ...
            <table class="admintable" style="width: 99%">
              ...
              <span class="clearfix"><?php echo JText::_('COM_JOBBOARD_CV_OR_RESUME') ?> :
                <a class="jobbrdmodal" href="<?php echo JFilterOutput::ampReplace('index.php?option=com_jobboard&view=user&pid=' . $this->pid . '&sid=' . $this->appl_uid . '&jid=' . $this->jid . '&tmpl=component') ?>">
                    <strong><?php echo $this->cv_name ?></strong> </a>
              </span>
                        ...
                        <select id="status" name="status">
                            <?php foreach ($this->statuses as $status) : ?>
                                <option value="<?php echo $status->id ?>" <?php if ($status->id == $appl_status) {
                                    echo 'selected="selected"';
                                } ?>><?php echo $status->status_description; ?></option>
                            <?php endforeach; ?>
                        </select> <label><?php echo JText::_('COM_JOBBOARD_LAST_UPDATED') ?></label>
                 ...
                 </span> <label class="clearfix" for="admin_notes">
                            <?php echo JText::_('COM_JOBBOARD_NOTES'); ?> (
                            <small><?php echo JText::_('COM_JOBBOARD_INTERNAL_USE'); ?>, <?php echo JText::_('COM_JOBBOARD_BACKOFFICE_ONLY'); ?></small>
