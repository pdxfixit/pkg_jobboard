...
class JobboardControllerImage extends JController {
    ...
    private function _uploadImage($img, $uid = 0, $post) {
        ...
        if ($user_profile_id > 0) {
            ...
            if ($imgdata->profile_image_present > 0) { //delete existing pic
        ...
        if ($img["size"] > 0) {
            if ($img["size"] > 1048576) { // Max filesize check
        ...
        } else if (strlen($img_name) <= 0 || $img["size"] <= 0) { // name length and zero-byte check
        ...
        $thumb_ratio = 1;
        $thumb48_width = 48;
        $thumb48_height = $thumb48_width;
        $thumb115_width = 115;
        $thumb115_height = $thumb115_width;
