...
<?php if ($this->config->use_location == 1) : ?>
    ...
    <?php $dist_option = false;
    if ($this->locsrch != '') $dist_option = true; ?>
<?php endif ?>
        ...
        <?php if ($user->get('guest')) : ?>
            <div class="jbdispnone" id="loginPopup">
                <form action="<?php echo JRoute::_('index.php?option=com_jobboard&view=member', true, JobBoardHelper::useSecure()); ?>" id="loginPopupForm" method="post">
                    ...
                    <div class="action">
                        <button type="submit" class="button"><span><?php echo JText::_('COM_JOBBOARD_LOGIN'); ?></span>
                        </button>
                    ...
                    <input type="hidden" name="view" value="member" /> <input type="hidden" name="task" value="login" />
...
<?php $link = 'index.php?option=com_jobboard&view=list&Itemid=' . $itemid; ?>
    ...
    <div id="srchInputs">
        ...
        <div id="jbrdadvsrch" class="jbdispnone">
            <label><?php echo JText::_('COM_JOBBOARD_ADVANCED_FILTERS') ?>
                <br class="clear" /> </label> <select name="selcat" id="fcats" class="inputfield jbrd-fleft">
                <?php foreach ($this->categories as $cat) : ?>
                    <option class="catitem" value="<?php echo $cat->id; ?>" <?php if ($cat->id == $this->selcat) {
                        $selcat = $cat->id;
                        $seldesc = $cat->type;
                ...
                <?php if ($this->rss_on) : ?>
                    ...
                    <?php $document->addHeadLink(JRoute::_($feedlink . '&type=rss') . $feedPrefix . 'format=feed', 'alternate', 'rel', $rss);
                    $document->addHeadLink(JRoute::_($feedlink . '&type=atom') . $feedPrefix . 'format=feed', 'alternate', 'rel', $atom); ?>
                <?php endif; ?>
                <?php $document->setTitle(JText::_('JOBS_IN') . ': ' . $seldesc); ?>
            </select> <label for="daterange" id="drcapt">
                <?php echo JText::_('JOBS_FROM') ?>
            </label> <select id="daterange" name="daterange" class="inputfield jbrd-fleft jbrd-mleft5">
            ...
            <div class="clear">&nbsp;</div>
            <label> <strong><?php echo JText::_('COM_JOBBOARD_ENT_REF') ?></strong>&nbsp;#&nbsp;
            </label> <input type="text" id="ref_num" value="" name="ref_num" maxlength="150" />

            <div class="clear">&nbsp;</div>
            <div class="clear">
                <label> <strong><?php echo JText::_('JOB_TYPE') ?></strong> </label>
                ...
                <div id="jtCboxes" class="checkRow">
                    ...
                    <?php for ($jt = 0; $jt < $num_jobtypes; $jt++)  : ?>
                        <input type="checkbox" id="job_type<?php echo $jt ?>" name="filter_job_type[]" value="<?php echo $jt ?>" <?php if (in_array($jt, $this->filter_job_type)) echo 'checked="checked"' ?> />
                        <label for="job_type<?php echo $jt ?>"><?php echo JText::_($this->jobtypes[$jt]) ?></label>
                    <?php endfor ?>
                </div>
            </div>
            <div class="clear">&nbsp;</div>
            <div id="clCboxes" class="clear">
                ...
                <div class="checkRow">
                    <div class="jbdcolumn">
                        <?php $num_clevels = count($this->jobcareerlvls);
                        $cl_count = 0;
                        $multicol = $num_clevels > 4 ? true : false; ?>
            ...
            <div id="elCboxes" class="clear">
                ...
                <div class="checkRow">
                    <div class="jbdcolumn">
                        <?php $num_elevels = count($this->jobedlvls);
                        $el_count = 0;
                        $multicol = $num_elevels > 4 ? true : false; ?>
    ...
    <?php if ($selcat <> 1 || $daterange <> 0 || $this->country_id <> 0) : ?>
        <div id="keywd_info" class="filterset">
            ...
            <?php endif ?>
            <?php if ($selcat <> 1) : ?>:
                <a id="jall" href="<?php echo JRoute::_($all_jobs); ?>" class="JobLink"><?php echo JText::_('COM_JOBBOARD_LIST_ALL_CATS'); ?></a><?php endif; ?>
        </div>
    <?php endif; ?>
    <div id="srch_info" class="filterset clear">
        ...
        <?php if (strlen($this->locsrch) > 0 && $this->geo_address <> '') echo JText::_('COM_JOBBOARD_JOBS_NEAR') . ' ' . ucfirst($this->geo_address) . ' &bull; '; ?>
        <a id="reset_keywds" href="<?php echo JRoute::_($reset_keywords); ?>" class="JobLink<?php if (strlen($this->jobsearch) == 0 && strlen($this->keysrch) == 0 && strlen($this->locsrch) == 0) echo ' jbhidden' ?>"><?php echo JText::_('COM_JOBBOARD_RESET_KEYWDS'); ?></a><br class="clear" />
        <?php if (!empty($this->filter_job_type) || !empty($this->filter_careerlvl) || !empty($this->filter_edulevel) || $this->country_id > 0 || $this->daterange > 0) : ?>
            <?php echo JText::_('COM_JOBBOARD_ADVANCED_FILTERS_ACTIVE'); ?>&nbsp;&bull;&nbsp;
            <a id="reset_advfilters" href="#"><?php echo JText::_('COM_JOBBOARD_RESET_ACTIVE_FILTERS'); ?></a>
        <?php endif ?>
    </div>
    ...
    <div id="jobtable">
        ...
        <table class="text">
            ...
            <tr class="headbg">
                ...
                <td class="jtitle first">
                    <a <?php if ($order == 'date') echo 'class="' . $sortimage . '"'; ?> href="<?php echo JRoute::_($date_sort . $item_id_segment); ?>"><?php echo JText::_('POSTED_ON') ?></a>
                </td><?php $title_sort = $sortlink . '&order=title' ?>

                <td class="jtitle">
                    <a <?php if ($order == 'title') echo 'class="' . $sortimage . '"'; ?> href="<?php echo JRoute::_($title_sort . $item_id_segment); ?>"><?php echo JText::_('TITLE'); ?></a>
                </td><?php $level_sort = $sortlink . '&order=level' ?>

                <td class="jtitle"><span><?php echo JText::_('COM_JOBBOARD_ENT_REF'); ?></span></td>
                <td class="jtitle">
                    <a <?php if ($order == 'level') echo 'class="' . $sortimage . '"'; ?> href="<?php echo JRoute::_($level_sort . $item_id_segment); ?>"><?php echo JText::_('CAREER_LEVEL'); ?></a>
                </td><?php $city_sort = $sortlink . '&order=city' ?>

                <?php if ($this->config->use_location == 1) : ?>
                    <?php $city_sort = $sortlink . '&order=city' ?>
                    <td class="jtitle">
                        <a <?php if ($order == 'city') echo 'class="' . $sortimage . '"'; ?> href="<?php echo JRoute::_($city_sort . $item_id_segment); ?>"><?php echo JText::_('LOCATION'); ?></a>
                    </td>
                <?php else: ?>
                    <?php if ($this->config->jobtype_coloring == 1) : ?>
                        <?php $jobtype_sort = $sortlink . '&order=jobtype' ?>
                        <td class="jtitle">
                            <a <?php if ($order == 'jobtype') echo 'class="' . $sortimage . '"'; ?> href="<?php echo JRoute::_($jobtype_sort . $item_id_segment); ?>"><?php echo JText::_('JOB_TYPE'); ?></a>
                        </td>
                    <?php else: ?>
                        <td class="jtitle">&nbsp;</td>
                    <?php endif ?>
                ...
                <td class="jtitle last">
                    <a <?php if ($order == 'type') echo 'class="' . $sortimage . '"'; ?> href="<?php echo JRoute::_($type_sort . $item_id_segment); ?>"><?php echo JText::_('CATEGORY'); ?></a>
                </td>
            </tr>
            <?php if ($count < 1) : ?>
                <tr>
                ...
                <?php foreach ($this->data as $row) : ?>
                    ...
                    <tr>
                        ...
                        <td class="<?php echo $row_style ?>">
                            <a href="<?php echo JRoute::_($job_link); ?>" class="JobLink" target="_top">
                                <?php if (strlen($this->jobsearch) > 0) : ?>
                                    <?php $pattern = $this->jobsearch;
                                    $replacement = '<span class="highlight">' . $this->jobsearch . '</span>'; ?>
                                ...
                                <?php if (strlen($this->keysrch) > 0) : ?>
                                    ...
                                    <?php foreach ($skillsets as $keywd) : ?>
                                        <?php $pattern = $keywd;
                                        $replacement = '<span class="highlight">' . $keywd . '</span>'; ?>
                                        <?php $job_title_h = str_ireplace($pattern, $replacement, $job_title_h); ?>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                                <?php if (strlen($this->locsrch) > 0) : ?>
                                    <?php $pattern = $this->locsrch;
                                    $replacement = '<span class="highlight">' . $this->locsrch . '</span>'; ?>
                                    <?php $job_title_h = str_ireplace($pattern, $replacement, $job_title_h); ?>
                                    <?php $city_h = str_ireplace($pattern, $replacement, $city_h); ?>
                                <?php endif; ?>
                                <strong><?php echo $job_title_h; ?></strong> </a>
                            <?php if ($this->config->use_location == 1 && isset($row->distance)) : ?>
                                <?php if ($row->distance > 0) : ?>
                                    ...
                                    <span class="">
                              <br /><strong>&plusmn;&nbsp;<?php echo $row->distance . ' ' . $this->dist_symbol ?>*</strong><?php echo $alt_distance ?>
                                        <br /><?php echo JText::sprintf('COM_JOBBOARD_FROM_LOCATION', '<strong>' . ucfirst($this->locsrch) . '</strong>'); ?>
                                        <br />
                        ...
                        <td class="<?php echo $row_style ?> jobsnippet">
                            <?php if ($this->config->use_location == 1) : ?>
                                <?php if ($row->country <> 266) echo $city_h; else echo JText::_('WORK_ANYWHERE'); ?>
                                <br />
                            <?php endif; ?>
                            <!-- job coloring -->
                            <?php if ($this->config->jobtype_coloring == 1) : ?>
        ...
        <div class="jbPagination">
            <span class="left"><?php echo JText::_('RESULTS_PER_PAGE') . ':&nbsp;&nbsp;' . $this->pagination->getLimitBox(); ?></span><?php echo $this->pagination->getPagesLinks() ?><!--  -->
        </div>
        <?php if ($this->rss_on) : ?>
            <div id="feedarea">
                <strong><?php echo JText::_('RSS') ?></strong> <?php echo JText::_('FEED'); ?>:
