...
<?php $link = 'index.php?option=com_jobboard&view=list&Itemid=' . $itemid; ?>
    ...
    <div id="jobtable">
        ...
        <div class="pagination"><?php echo $this->pagination->getResultsCounter(); ?>&nbsp;&nbsp;<a href="<?php echo JRoute::_($tbl_view_link) ?>" title="<?php echo JText::_('COM_JOBBOARD_LIST_TBLVIEW') ?>" id="tableView" class="tableView"></a><a title="<?php echo JText::_('COM_JOBBOARD_LIST_LISTVIEW') ?> (<?php echo JText::_('COM_JOBBOARD_LIST_CURRVIEW') ?>)" class="listView active"></a>
        </div>
        <br class="clear" />
        <table class="text">
            <tbody>
                ...
                <td id="listSort">
                    <?php if ($this->rss_on) : ?>
                        ...
                        <?php if ($rss_tag <> '' && count(explode(',', $rss_tag)) == 1) : ?>
                            ...
                            <small>
                                <a class="sFeedicon" href="<?php echo JRoute::_($tag_feedlink) . $feedPrefix . 'format=feed' ?>"><?php echo JText::_('JOBS_WITH') . ' "' . ucfirst($rss_tag) . '" ' . JText::_('KEYWD_TAG'); ?></a>
                            </small>
                        <?php endif; ?>
                    <?php endif ?>
            ...
            <?php else: ?>
                ...
                <?php foreach ($this->data as $row) : ?>
                    ...
                    <?php endif; ?>
                    <!-- end job coloring -->
                    <?php if ($row->featured == 1) $row_style .= ' featured';
                    $featured_incr += 1; ?>
                    ...
                    <tr>
                        <td class="<?php echo $row_style ?> jobsnippet fleft">
                            ...
                            <?php if (strlen($full_desc) > 400) {
                                $chopped_desc = substr($full_desc, 0, 400) . '... ';
                            } else $chopped_desc = $full_desc; ?>
                        ...
                        <td class="<?php echo $row_style ?> fright">
                            <ul>
                                <li class="lstsumm SummaryRow"><?php if ($order == 'date') echo '<span class="' . $sortimage . '">&nbsp;</span>'; ?><?php $date = new JDate($row->post_date); ?>
                                    <b><?php echo JText::_('POSTED_ON'); ?>
                                ...
                                <?php if ($this->config->use_location == 1 && isset($row->distance)) : ?>
                                    ...
                                    <?php if ($row->distance > 0) : ?>
                                        ...
                                        <li class="lstsumm SummaryRow secondLast distance"><?php if ($order == 'distance') echo '<span class="' . $sortimage . '">&nbsp;</span>'; ?>
                                            <strong>&plusmn;&nbsp;<?php echo $row->distance . ' ' . $this->dist_symbol ?>*</strong><?php echo $alt_distance ?>&nbsp;<?php echo JText::sprintf('COM_JOBBOARD_FROM_LOCATION', '<strong>' . ucfirst($this->locsrch) . '</strong>'); ?>
                                            <br />
                                ...
                                <li class="lstsumm lastRow">
                                    <button class="button" onclick="<?php echo "event.preventDefault();window.location.href='" . $routed_link ?>"><?php echo JText::_('SEE_DETAILS'); ?></button>
                                </li>
                            </ul>
                        </td>
                    </tr>
